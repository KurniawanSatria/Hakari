...
// BUG FIX: always use player.queue.tracks (the actual array) for index math;
// fall back to an empty array so nothing throws.
...
async function saveQueueMsg(message, player) {
    try {
        const sent = message.fetchReply ? await message.fetchReply() : message;
...
function buildTrackComponents(track, positionLabel, waitFormatted, totalInQueue) {
    const thumbnail = new ThumbnailBuilder().setURL(track.thumbnail || FALLBACK_TRACK_IMG);
...
function buildPlaylistComponents(playlistName, tracks, startPosition, waitFormatted, totalInQueue) {
    ...
    const thumbnail = new ThumbnailBuilder().setURL(tracks[0]?.thumbnail || FALLBACK_TRACK_IMG);
...
module.exports = {
    ...
    execute: async (client, message, args) => {
        try {
            ...
            // Connect first so search can be attributed to the right node
            ...
            if (loadType === "playlist") {
                const queueArr = getQueueArray(player);
                ...
                const totalInQueue = getQueueArray(player).length;
                const isIdle = !player.playing && !player.paused;
                const waitMs = calcWaitTime(player, startPosition);
                const waitFormatted = isIdle ? "Now" : formatDuration(waitMs);
                const playlistName = playlistInfo?.name || tracks[0]?.playlist?.name || "Unknown Playlist";
            ...
            // ─── SINGLE TRACK / SEARCH ───────────────────────────────────────
                ...
                const positionIndex = getQueueArray(player).length;
                player.queue.add(track);
                ...
                const waitFormatted = isIdle ? "Now" : `~${formatDuration(waitMs)}`;
                const positionLabel = isIdle ? "Up next" : `#${positionIndex + 1}`;
            ...
            // ─── NO RESULTS ──────────────────────────────────────────────────
                ...
                return message.reply({ content: "No results found for that query." });
            }
            ...
            await message.reply({ content: "An error occurred while trying to play that track." }).catch(() => {});
        }
    },
};